...
--[[
	...
	Comprehensive Game Dump feature for Dex.
	
	Recursively traverses game, CoreGui, and nil-parented instances
	(via getnilinstances / getinstances when available). Decompiles every
	LocalScript / ModuleScript / Script via the executor's decompile() API,
	and mirrors the Explorer tree on disk using writefile() / makefolder().

	Output root:
		dex/dumps/<PlaceId>_<timestamp>/
			game/
				Workspace/
					...
				ReplicatedStorage/
	...
	Each instance (including non-scripts) gets a sibling `<Name>.info.txt`
	file describing its class and a small set of common properties. Script
	instances additionally get a `<Name>.lua` containing the decompiled
	source (or the best-effort fallback source / error marker).
]]
...
local function main()
	...
	-- Yield to the scheduler every N instances so we don't freeze Roblox.
	local YIELD_EVERY = 250

	-- Maximum filename length for a single path component. Windows path
	-- components cap at 255; we stay well below to leave headroom for
	-- long parent chains.
	local MAX_COMPONENT_LEN = 120

	-- Properties we attempt to read for the `.info.txt` sidecar files.
	-- Reading is wrapped in pcall because a lot of these will error on
	-- instances that don't have them, or on locked/security-filtered
	-- properties (we swallow the error and move on).
	local INFO_PROPERTIES = {
		"ClassName", "Name", "Archivable",
		-- BasePart / physical
		"Position", "Size", "CFrame", "Orientation", "Anchored", "CanCollide",
		"Material", "Color", "Transparency", "Reflectance",
		-- Gui
		"Visible", "AbsolutePosition", "AbsoluteSize", "ZIndex", "Text",
		-- Script
		"Disabled", "RunContext",
		-- Mesh / Attachment / Model
		"MeshId", "TextureId", "AssetId", "SoundId", "PrimaryPart",
		-- Value objects
		"Value",
	}
	...
	-------------------------------------------------------------------
	-- State (reset per dump run)
	-------------------------------------------------------------------

	local running = false
	local cancelRequested = false
	local stats, logLines, window, guiElems
	...
	-- Characters forbidden on Windows filesystems (and generally a bad
	-- idea on POSIX too). NUL is always forbidden. We also strip control
	-- characters below 0x20.
	local FORBIDDEN_PATTERN = "[%z\1-\31<>:\"/\\|%?%*]"

	-- Reserved Windows device names that cannot be used as bare
	...
	local function sanitizeComponent(name)
		...
		-- Trim leading/trailing whitespace and dots (Windows strips
		-- these silently and the resulting name can collide).
		...
		local stem = string.match(upper, "^([^%.]+)") or upper
		if RESERVED_NAMES[stem] then
			name = "_" .. name
		end
	...
	-------------------------------------------------------------------
	-- Utility: environment capability checks
	-------------------------------------------------------------------
	...
	local function getDecompiler()
		-- env.decompile was populated from the global `decompile` at init
		-- time. Fall back to the raw global in case the executor exposes
		-- it late or under a different name.
		if type(env.decompile) == "function" then return env.decompile end
		local g = getfenv(0)
		if type(rawget(g, "decompile")) == "function" then
			return rawget(g, "decompile")
		end
		return nil
	end

	-- getinstances() returns every instance the executor knows about,
	-- including parented ones. We use it as a superset when available
	-- but still traverse game/CoreGui explicitly so the output tree is
	-- rooted predictably.
	local function tryGetAllInstances()
		local g = getfenv(0)
		local fn = rawget(g, "getinstances")
	...
	local function tryGetNilInstances()
		if type(env.getnilinstances) == "function" then
			local ok, res = pcall(env.getnilinstances)
			if ok and type(res) == "table" then return res end
		end
		return nil
	end
	...
	-------------------------------------------------------------------
	-- Utility: folder creation with memoization
	-------------------------------------------------------------------

	local createdFolders = {}

	-- makefolder() in most executors only creates a single directory,
	-- not a chain. We walk the chain manually and memoize.
	local function ensureFolder(path)
		if createdFolders[path] then return true end
		...
		createdFolders[path] = true -- Memoize regardless; makefolder is
		                            -- idempotent on existing dirs and we
		                            -- don't want to retry every call.
		return ok
	end

	-------------------------------------------------------------------
	-- Utility: safe property read + info-file serialization
	-------------------------------------------------------------------

	local function safeIndex(inst, prop)
	...
	local function valueToString(v)
		...
		if t == "Instance" then
			...
			return ok and ("Instance: " .. full) or "Instance: <unreadable>"
		elseif t == "EnumItem" then
			return "Enum." .. tostring(v.EnumType) .. "." .. v.Name
		elseif t == "string" then
			-- Keep short strings inline, truncate long ones. Escape
			-- newlines so the sidecar stays one-property-per-line.
			if #v > 200 then v = string.sub(v, 1, 200) .. "..." end
		...
		elseif t == "boolean" or t == "number" then
			...
			return ok and (t .. ": " .. s) or t .. ": <unreadable>"
		end
	end

	local function buildInfoText(inst, fullPath)
		local lines = {}
		lines[#lines + 1] = "-- Dex Dump Info File"
		lines[#lines + 1] = "FullName: " .. tostring(fullPath)
		for _, prop in ipairs(INFO_PROPERTIES) do
			local val, errMarker = safeIndex(inst, prop)
			if errMarker then
				-- property access errored; skip rather than include noise
			elseif val ~= nil then
				lines[#lines + 1] = prop .. " = " .. valueToString(val)
			end
		end
	...
	local function extractScriptSource(inst, decompile)
		-- Prefer a direct .Source read first — on some executors
		-- reading .Source from a LocalScript/ModuleScript just works
		-- and is cheaper than decompile().
		...
		return "-- Dex Dumper: no source available (decompile failed or unsupported)",
			"unavailable"
	end

	-------------------------------------------------------------------
	-- Utility: logging into the window
	-------------------------------------------------------------------

	local function log(msg)
		logLines[#logLines + 1] = tostring(msg)
		-- Bound the log so it doesn't explode on massive games.
		if #logLines > 500 then
			table.remove(logLines, 1)
		end
		if guiElems and guiElems.LogLabel then
			guiElems.LogLabel.Text = table.concat(logLines, "\n")
			-- Scroll to bottom.
			local scroll = guiElems.LogScroll
			if scroll then
	...
	local function updateStatus()
		if not guiElems or not guiElems.StatusLabel then return end
		guiElems.StatusLabel.Text = string.format(
			"Visited: %d   Scripts: %d   Files: %d   Errors: %d",
			stats.visited or 0, stats.scripts or 0,
			stats.files or 0, stats.errors or 0
		)
	end
	...
	-- Pre-allocate a cache mapping instance -> already-chosen path
	-- component, so siblings with identical names (e.g. multiple
	-- unnamed Frames) get stable, non-colliding suffixes.
	local function buildChildPathMap(parent, usedNames)
		-- usedNames is a table { [sanitizedName] = count } scoped to
		-- one parent. On each call we increment the count and append
		-- _2, _3, ... if there's already an entry.
		return function(child)
			local base = sanitizeComponent(child.Name)
			local classSuffix = ""
			-- Empty or very generic names get the class name appended
			-- to make the tree more browseable.
			if child.Name == "" then
				local ok, cn = pcall(function() return child.ClassName end)
				classSuffix = ok and ("_" .. sanitizeComponent(cn)) or ""
				base = base .. classSuffix
			end
	...
	local visited -- populated per-run

	local function dumpInstance(inst, folderPath)
		if cancelRequested then return end
		if not inst or visited[inst] then return end
		visited[inst] = true

		stats.visited = stats.visited + 1

		local okClass, className = pcall(function() return inst.ClassName end)
		if not okClass then className = "Unknown" end

		-- Write the sidecar info file for this instance. Placed INSIDE
		-- the instance's own folder so you can see properties without
		-- leaving the directory.
		ensureFolder(folderPath)

		local okName, nm = pcall(function() return inst.Name end)
		local displayName = okName and nm or "<unnamed>"

		local fullPathOk, fullPathStr = pcall(function() return inst:GetFullName() end)
		if not fullPathOk then fullPathStr = displayName end

		local infoOk = pcall(function()
			env.writefile(
				folderPath .. "/.info.txt",
				buildInfoText(inst, fullPathStr)
			)
		end)
		if infoOk then
		...
		-- If this is a script, pull the source.
		if SCRIPT_CLASSES[className] then
			local decompile = getDecompiler()
			local source, kind = extractScriptSource(inst, decompile)
			local scriptFileName = sanitizeComponent(displayName) .. ".lua"

			-- Prepend a small header so you know where the source came
			-- from and whether it was a successful decompile.
			local header = string.format(
				"-- %s\n-- FullName: %s\n-- Source kind: %s\n\n",
				className, tostring(fullPathStr), kind
			)

			local writeOk = pcall(function()
				env.writefile(folderPath .. "/" .. scriptFileName, header .. source)
			end)
			if writeOk then
				...
				stats.errors = stats.errors + 1
				log("Failed writing script: " .. tostring(fullPathStr))
			end
		end

		-- Periodic yield + UI refresh.
		...
		local okChildren, children = pcall(function() return inst:GetChildren() end)
		if not okChildren or type(children) ~= "table" then return end

		local usedNames = {}
		local getChildPath = buildChildPathMap(inst, usedNames)

		for i = 1, #children do
			local child = children[i]
			if not visited[child] then
				local childFolder = folderPath .. "/" .. getChildPath(child)
				dumpInstance(child, childFolder)
				if cancelRequested then return end
			end
		end
	end
	...
	local function runDump()
		...
		if not hasFilesystemAPI() then
			log("ERROR: Executor does not expose writefile/makefolder. Cannot dump.")
			return
		end
		...
		visited = {}
		createdFolders = {}
		stats = { visited = 0, scripts = 0, files = 0, errors = 0 }
		logLines = {}
		...
		-- 1. game tree
		log("Traversing game...")
		dumpInstance(game, root .. "/game")

		-- 2. CoreGui is already reached via game traversal above — it's
		-- a direct child of game — so we don't need a second pass. This
		-- block is left as a safety net for edge cases where CoreGui is
		-- reparented or unreachable from game (rare, but possible on
		-- some patched executors).
		if not cancelRequested then
			local okCore, coreGui = pcall(function()
				return service.CoreGui
			end)
			if okCore and coreGui and not visited[coreGui] then
				log("Traversing CoreGui (fallback pass — not reached via game)...")
				dumpInstance(coreGui, root .. "/CoreGui")
			end
		end

		-- 3. Nil-parented instances.
		if not cancelRequested then
			local nilList = tryGetNilInstances()
			if nilList then
				log(string.format("Traversing %d nil instances...", #nilList))
				...
				for i = 1, #nilList do
					...
					if inst and not visited[inst] and inst ~= game then
						counter = counter + 1
						local okCN, cn = pcall(function() return inst.ClassName end)
						local okNm, nm = pcall(function() return inst.Name end)
						local folderName = string.format(
							"%04d_%s_%s",
							counter,
							sanitizeComponent(okCN and cn or "Unknown"),
							sanitizeComponent(okNm and nm or "unnamed")
						)
						dumpInstance(inst, nilRoot .. "/" .. folderName)
						if cancelRequested then break end
					end
				end
			else
				log("getnilinstances() unavailable — skipping nil pass.")
			end
		end

		-- 4. Sweep via getinstances() for anything neither rooted under
		-- game/CoreGui nor reported as nil (some executors stash wrapped
		-- instances that aren't in either set).
		if not cancelRequested then
			local all = tryGetAllInstances()
			if all then
				log(string.format("Sweeping %d instances from getinstances()...", #all))
				...
				for i = 1, #all do
					...
					if inst and not visited[inst] then
						...
						dumpInstance(inst, sweepRoot .. "/" .. folderName)
						if cancelRequested then break end
					end
				end
			end
		...
		if cancelRequested then
			...
			log("Dump complete.")
			log(string.format(
				"Totals: %d instances, %d scripts, %d files written, %d errors.",
	...
	Dumper.Init = function()
		...
		desc.Text = "Dumps the full game tree (game, CoreGui, nil instances) to "
			.. "dex/dumps/<PlaceId>_<time>/. Scripts are decompiled to .lua files, "
			.. "other instances get .info.txt sidecars."
		desc.Parent = content
		...
		logScroll.Size = UDim2.new(1, -16, 1, -110)
		...
		startBtn.MouseButton1Click:Connect(function()
			...
			-- Kick off the dump in a coroutine so the UI thread stays
			-- responsive. runDump yields on Lib.FastWait periodically.
			activeCoroutine = coroutine.wrap(runDump)
			activeCoroutine()
		end)

		-- "Open Folder" button — attempts to print the output path to
		-- console so the user can copy/paste it. Some executors expose
		-- openfolder / pcall to discover.
		local openBtn = Instance.new("TextButton")
		openBtn.Position = UDim2.new(0, 176, 1, -32)
		openBtn.Size = UDim2.new(0, 160, 0, 24)
		openBtn.BackgroundColor3 = Settings.Theme.Button
		openBtn.BorderColor3 = Settings.Theme.Outline2
		openBtn.TextColor3 = Settings.Theme.Text
		openBtn.Font = Enum.Font.SourceSans
		openBtn.TextSize = 14
		openBtn.AutoButtonColor = true
		openBtn.Text = "Copy Path to Clipboard"
		...
		openBtn.MouseButton1Click:Connect(function()
			...
			if type(env.setclipboard) == "function" then
				...
				log("Copied path pattern to clipboard: " .. path)
			else
				log("Path pattern (no clipboard available): " .. path)
			end
		end)
	end

	Dumper.Dump = function()
		-- Public API: lets other modules / plugins kick off a dump
		-- without opening the window. Useful for automation.
