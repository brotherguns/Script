...
Main = (function()
	...
	Main.LoadModules = function()
		for i,v in pairs(Main.ModuleList) do
			...
			if not s then Main.Error("FAILED LOADING "..v.." CAUSE "..tostring(e)) end
		end
		Explorer = Apps.Explorer
		Properties = Apps.Properties
		ScriptViewer = Apps.ScriptViewer
	...
	Main.InitEnv = function()
		...
		env.makefolder = getG("makefolder")
		env.listfiles  = getG("listfiles")
		env.loadfile   = getG("loadfile")
		env.saveinstance = getG("saveinstance")
		local dbg = getG("debug") or {}
		env.getupvalues  = (type(dbg)=="table" and dbg.getupvalues) or getG("getupvals")
		env.getconstants = (type(dbg)=="table" and dbg.getconstants) or getG("getconsts")
		env.islclosure   = getG("islclosure") or getG("is_l_closure")
		env.checkcaller  = getG("checkcaller")
		env.getreg       = getG("getreg")
		env.getgc        = getG("getgc")
		env.setfflag     = getG("setfflag")
		env.decompile    = getG("decompile")
		local pg = getG("protect_gui")
		local syn = getG("syn")
		env.protectgui   = pg or (type(syn)=="table" and syn.protect_gui)
		env.gethui       = getG("gethui")
		env.setclipboard = getG("setclipboard")
		env.getnilinstances  = getG("getnilinstances") or getG("get_nil_instances")
		env.getloadedmodules = getG("getloadedmodules")
	...
	Main.FetchAPI = function()
		...
		if Main.Elevated then
			if Main.LocalDepsUpToDate() then
				local ok,f = pcall(Lib.ReadFile,"dex/rbx_api.dat")
				if ok and f then rawAPI = f end
			end
			if not rawAPI then
				local ok,r = pcall(function() return game:HttpGet("http://setup.roblox.com/"..Main.RobloxVersion.."-API-Dump.json") end)
				if ok then rawAPI = r end
			end
		end
		if not rawAPI then
			local ok,r = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/MaximumADHD/Roblox-Client-Tracker/roblox/DeployHistory.txt") end)
		end
		if not rawAPI or rawAPI == "" then
			return {Classes={},Enums={},CategoryOrder={},GetMember=function() return {} end}
		end
		Main.RawAPI = rawAPI
		local ok,apiData = pcall(function() return service.HttpService:JSONDecode(rawAPI) end)
		...
		local function insertAbove(t,item,aboveItem)
			...
			table.remove(t,fp)
			local pos = table.find(t,aboveItem); if not pos then return end
			table.insert(t,pos,item)
		end
		for _,class in pairs(apiData.Classes or {}) do
			local newClass = {Name=class.Name,Superclass=class.Superclass,Properties={},Functions={},Events={},Callbacks={},Tags={}}
			if class.Tags then for c,tag in pairs(class.Tags) do newClass.Tags[tag]=true end end
			for __,member in pairs(class.Members or {}) do
				local newMember = {Name=member.Name,Class=class.Name,Security=member.Security,Tags={}}
				...
				if mType == "Property" then
					...
					if not seenCategories[cat] then categoryOrder[#categoryOrder+1]=cat; seenCategories[cat]=true end
					newMember.ValueType=member.ValueType; newMember.Category=cat; newMember.Serialization=member.Serialization
					table.insert(newClass.Properties,newMember)
				elseif mType == "Function" then
					newMember.Parameters={}
					newMember.ReturnType=member.ReturnType and member.ReturnType.Name
					for c,p in pairs(member.Parameters or {}) do table.insert(newMember.Parameters,{Name=p.Name,Type=p.Type and p.Type.Name}) end
		...
		for _,enum in pairs(apiData.Enums or {}) do
			local ne={Name=enum.Name,Items={},Tags={}}
			if enum.Tags then for c,tag in pairs(enum.Tags) do ne.Tags[tag]=true end end
			for __,item in pairs(enum.Items or {}) do table.insert(ne.Items,{Name=item.Name,Value=item.Value}) end
			enums[enum.Name]=ne
		end
		local function getMember(class,member)
			...
			while cur do for _,e in pairs(cur[member]) do result[#result+1]=e end cur=cur.Superclass end
			table.sort(result,function(a,b) return a.Name<b.Name end)
			return result
		end
		insertAbove(categoryOrder,"Behavior","Tuning"); insertAbove(categoryOrder,"Appearance","Data")
		insertAbove(categoryOrder,"Attachments","Axes"); insertAbove(categoryOrder,"Cylinder","Slider")
		insertAbove(categoryOrder,"Localization","Jump Settings"); insertAbove(categoryOrder,"Surface","Motion")
		insertAbove(categoryOrder,"Surface Inputs","Surface"); insertAbove(categoryOrder,"Part","Surface Inputs")
		insertAbove(categoryOrder,"Assembly","Surface Inputs"); insertAbove(categoryOrder,"Character","Controls")
		categoryOrder[#categoryOrder+1]="Unscriptable"; categoryOrder[#categoryOrder+1]="Attributes"
		local com={}; for i=1,#categoryOrder do com[categoryOrder[i]]=i end
		return {Classes=classes,Enums=enums,CategoryOrder=com,GetMember=getMember}
	end
	...
	Main.FetchRMD = function()
		...
		if Main.Elevated then
			if Main.LocalDepsUpToDate() then
				local ok,f = pcall(Lib.ReadFile,"dex/rbx_rmd.dat")
				if ok and f then rawXML = f end
			end
		end
		if not rawXML then
			local ok,r = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/CloneTrooper1019/Roblox-Client-Tracker/roblox/ReflectionMetadata.xml") end)
			if ok then rawXML = r end
		end
		Main.RawRMD = rawXML or ""
		if not rawXML or rawXML=="" then return {Classes={},Enums={},PropertyOrders={}} end
		local ok,parsed = pcall(function() return Lib.ParseXML(rawXML) end)
		if not ok or not parsed then return {Classes={},Enums={},PropertyOrders={}} end
		local c1 = parsed.children and parsed.children[1]
		local classList = c1 and c1.children and c1.children[1] and c1.children[1].children or {}
		local enumList  = c1 and c1.children and c1.children[2] and c1.children[2].children or {}
		local propertyOrders={}; local classes={}; local enums={}
		for _,class in pairs(classList) do
			local className=""
			for _,child in pairs(class.children or {}) do
				if child.tag=="Properties" then
					local data={Properties={},Functions={}}
					for _,prop in pairs(child.children or {}) do
						local n=prop.attrs and prop.attrs.name
						if n then
							n=n:sub(1,1):upper()..n:sub(2)
							data[n]=prop.children and prop.children[1] and prop.children[1].text
						end
					end
					className=data.Name or ""; classes[className]=data
				elseif child.attrs and child.attrs.class=="ReflectionMetadataProperties" then
					for _,member in pairs(child.children or {}) do
						if member.attrs and member.attrs.class=="ReflectionMetadataMember" then
							local data={}
							if member.children and member.children[1] and member.children[1].tag=="Properties" then
								for _,prop in pairs(member.children[1].children or {}) do
									if prop.attrs then
										local n=prop.attrs.name; n=n:sub(1,1):upper()..n:sub(2)
										data[n]=prop.children and prop.children[1] and prop.children[1].text
									end
								end
								if data.PropertyOrder and classes[className] then
								...
								if classes[className] then classes[className].Properties[data.Name]=data end
							end
						end
					end
				end
		...
		for _,enum in pairs(enumList) do
			local enumName=""
			for _,child in pairs(enum.children or {}) do
				if child.tag=="Properties" then
					local data={Items={}}
					for _,prop in pairs(child.children or {}) do
						local n=prop.attrs and prop.attrs.name
						if n then n=n:sub(1,1):upper()..n:sub(2); data[n]=prop.children and prop.children[1] and prop.children[1].text end
					end
					enumName=data.Name or ""; enums[enumName]=data
				elseif child.attrs and child.attrs.class=="ReflectionMetadataEnumItem" then
					local data={}
					if child.children and child.children[1] and child.children[1].tag=="Properties" then
						for _,prop in pairs(child.children[1].children or {}) do
							if prop.attrs then
								local n=prop.attrs.name; n=n:sub(1,1):upper()..n:sub(2)
								data[n]=prop.children and prop.children[1] and prop.children[1].text
							end
						end
						if enums[enumName] then enums[enumName].Items[data.Name]=data end
					end
				end
			end
		end
	...
	Main.CreateIntro = function(initStatus)
		local gui = create({
			...
			{13,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Version",Parent={2},Position=UDim2.new(1,-110,1,-35),Size=UDim2.new(0,105,0,20),Text="Enhanced 2.0",TextColor3=Color3.new(1,1,1),TextSize=14,TextXAlignment=1,}},
		...
		local function fw(s) if s then task.wait(s) else task.wait() end end

		statusText.Text = initStatus

		local function tweenNumber(n,ti,func)
			local tv = Instance.new("IntValue"); tv.Value = 0
			tv.Changed:Connect(func)
			local tw = tweenS:Create(tv,ti,{Value=n}); tw:Play()
			tw.Completed:Connect(function() tv:Destroy() end)
		end
		...
		fw(0.9)
		local pti=TweenInfo.new(0.25,Enum.EasingStyle.Quad,Enum.EasingDirection.Out)
		tweenS:Create(statusText,pti,{Position=UDim2.new(0,20,0,120),TextTransparency=0}):Play()
		tweenS:Create(progressBar,pti,{Position=UDim2.new(0,60,0,145),Size=UDim2.new(0,100,0,4)}):Play()
		fw(0.25)

		local function setProgress(text,n)
			statusText.Text=text
			tweenS:Create(progressBar.Bar,pti,{Size=UDim2.new(n,0,1,0)}):Play()
		end
		local function close()
			tweenS:Create(titleText,pti,{TextTransparency=1}):Play()
			tweenS:Create(descText,pti,{TextTransparency=1}):Play()
			tweenS:Create(versionText,pti,{TextTransparency=1}):Play()
			tweenS:Create(creatorText,pti,{TextTransparency=1}):Play()
			tweenS:Create(statusText,pti,{TextTransparency=1}):Play()
			tweenS:Create(progressBar,pti,{BackgroundTransparency=1}):Play()
			tweenS:Create(progressBar.Bar,pti,{BackgroundTransparency=1}):Play()
			tweenS:Create(progressBar.ImageLabel,pti,{ImageTransparency=1}):Play()
	...
	Main.CreateApp = function(data)
		...
		if data.IconMap and iconIndex then
			if type(iconIndex)=="number" then data.IconMap:Display(app.Main.Icon,iconIndex)
			elseif type(iconIndex)=="string" then data.IconMap:DisplayByKey(app.Main.Icon,iconIndex) end
		elseif type(iconIndex)=="string" then
	...
	Main.SetMainGuiOpen = function(val)
		...
		Main.MainGui.OpenButton.MainFrame:TweenSize(val and UDim2.new(0,224,0,250) or UDim2.new(0,0,0,0),Enum.EasingDirection.Out,Enum.EasingStyle.Quad,0.2,true)
		...
		if not val then
			local startTime=tick(); Main.MainGuiCloseTime=startTime
			coroutine.wrap(function()
				Lib.FastWait(0.2)
	...
	Main.CreateMainGui = function()
		local gui = create({
			...
			{3,"UICorner",{CornerRadius=UDim.new(0,4),Parent={2},}},
			{4,"Frame",{AnchorPoint=Vector2.new(0.5,0),BackgroundColor3=Color3.new(0.17647059261799,0.17647059261799,0.17647059261799),ClipsDescendants=true,Name="MainFrame",Parent={2},Position=UDim2.new(0.5,0,1,-4),Size=UDim2.new(0,224,0,250),}},
			{5,"UICorner",{CornerRadius=UDim.new(0,4),Parent={4},}},
			...
			{14,"ScrollingFrame",{Active=true,AnchorPoint=Vector2.new(0.5,0),BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,BorderSizePixel=0,Name="AppsFrame",Parent={4},Position=UDim2.new(0.5,0,0,50),ScrollBarImageColor3=Color3.new(0,0,0),ScrollBarThickness=4,Size=UDim2.new(0,222,1,-74),}},
		...
		local dumpBtn = createSimple("TextButton",{
			...
			BackgroundColor3=Color3.fromRGB(40,110,40),
			BorderSizePixel=0,
			...
			Text="Dump Game",
			ZIndex=5,
			Parent=gui.OpenButton.MainFrame,
		})
		createSimple("UICorner",{CornerRadius=UDim.new(0,4),Parent=dumpBtn})
	...
	Main.DumpGame = function()
		...
		local SKIP_CLASS={Motor6D=true,Bone=true,WeldConstraint=true,Weld=true,RigidConstraint=true,NoCollisionConstraint=true,SpecialMesh=true,DataModelMesh=true,BlockMesh=true,CylinderMesh=true,FileMesh=true,Animator=true,AnimationController=true,ForceField=true,SelectionSphere=true,SelectionBox=true,ArcHandles=true,Handles=true,BallSocketConstraint=true,HingeConstraint=true,PrismaticConstraint=true,RodConstraint=true,RopeConstraint=true,SpringConstraint=true,TorsionSpringConstraint=true,UniversalConstraint=true,PlaneConstraint=true,LinearVelocity=true,AngularVelocity=true,Attachment=true,Decal=true,Texture=true,SurfaceAppearance=true,Fire=true,Smoke=true,Sparkles=true}
		local SKIP_NAME={Animate=true,RigAttachments=true,AvatarPartScaleType=true,PlayerModule=true,RbxCharacterSounds=true,AtomicBinding=true,TopbarStandard=true,TopbarStandardClipped=true,TopbarPlusGui=true,TopbarPlusReference=true,CorePackages=true}
		local FULL_DUMP={RemoteEvent=true,RemoteFunction=true,BindableEvent=true,BindableFunction=true,ProximityPrompt=true,ClickDetector=true,Script=true,LocalScript=true,ModuleScript=true,Sound=true,SoundGroup=true,SpawnLocation=true,Tool=true,NumberValue=true,StringValue=true,BoolValue=true,IntValue=true,ObjectValue=true,Vector3Value=true,Color3Value=true,CFrameValue=true,RayValue=true,ScreenGui=true,SurfaceGui=true,BillboardGui=true,Model=true,Configuration=true,Folder=true}
		local LITE_PROPS={Part={"Position","Size","Color","Transparency","Anchored","CanCollide"},MeshPart={"Position","Size","Color","Transparency","Anchored","CanCollide"},Humanoid={"Health","MaxHealth","WalkSpeed","JumpPower","DisplayName"},Frame={"Visible","BackgroundTransparency"},TextLabel={"Text","Visible"},TextButton={"Text","Visible"},TextBox={"Text","PlaceholderText"},ImageLabel={"Visible","Image"},ImageButton={"Visible","Image"}}
		local ALWAYS_KEEP={Enabled=true,Position=true,Size=true,Text=true,ActionText=true,ObjectText=true,Value=true,Health=true,HoldDuration=true,MaxActivationDistance=true,SoundId=true,WalkSpeed=true,JumpPower=true,Disabled=true,Name=true,DisplayName=true,Image=true,Volume=true,Looped=true}
		...
		local function sv(val)
			...
			elseif t=="Instance" then local ok,fn=pcall(function() return val:GetFullName() end); return "[ref]"..(ok and fn or "?")
			else return "["..t.."]" end
		end

		local buf={} local bl=0 local firstWrite=true
		local function fl()
		...
		local function getProps(cn)
			...
			for _,prop in ipairs(raw) do
				if not prop.Tags.WriteOnly and not prop.Tags.Hidden and not prop.Tags.Deprecated then f[#f+1]=prop.Name end
			end
			propCache[cn]=f; return f
		end
		...
		local function dumpInst(inst,depth,pad)
			...
			local ok,cn=pcall(function() return inst.ClassName end); if not ok then return end
			...
			local isScript=cn=="Script" or cn=="LocalScript" or cn=="ModuleScript"
			local isRemote=cn=="RemoteEvent" or cn=="RemoteFunction" or cn=="BindableEvent" or cn=="BindableFunction"
			local isSound=cn=="Sound"
			if isRemote then local ok3,fn=pcall(function() return inst:GetFullName() end); remotes[#remotes+1]="  ["..cn.."] "..(ok3 and fn or nm) end
			if isSound then local sid=""; pcall(function() sid=inst.SoundId end); sounds[#sounds+1]="  "..nm.." => "..sid end
			local pts=FULL_DUMP[cn] and getProps(cn) or LITE_PROPS[cn]
			if pts then
				local pl={}
				for _,pn in ipairs(pts) do
					if nm~=pn or ALWAYS_KEEP[pn] then
						local pok,pv=pcall(function() return inst[pn] end)
						if pok and pv~=nil then pl[#pl+1]=pad.."  "..pn.."="..sv(pv) end
					end
				end
				w(line); for _,pl2 in ipairs(pl) do w(pl2) end
			else
				w(line)
			end
			if Settings.Dump.DumpAttributes then
				local ok3,attrs=pcall(function() return inst:GetAttributes() end)
				if ok3 and attrs then
					for k,v in pairs(attrs) do w(pad.."  [A] "..tostring(k).."="..sv(v)) end
				end
			end
			if isScript and Settings.Dump.SaveScripts then scripts[#scripts+1]=inst end
			local cp=pad.."  "; local cc=0
			local ok4,ch=pcall(function() return inst:GetChildren() end)
			if ok4 and ch then
				for _,c in ipairs(ch) do
					cc=cc+1; if cc%YIELD_EVERY==0 then task.wait() end
					dumpInst(c,depth+1,cp)
				end
			end
		end

		local SERVICES={"ReplicatedStorage","ReplicatedFirst","Workspace","Players","StarterGui","StarterPack","StarterPlayer","ServerStorage","ServerScriptService","Teams","SoundService","Lighting","Chat","LocalizationService"}

		hdr("DEX GAME DUMP | PlaceId:"..tostring(game.PlaceId).." | "..os.date())
		w("")
		local ic=0
		for _,svcName in ipairs(SERVICES) do
			...
			if ok and svc then
				...
				if ok2 and ch then
					for _,c in ipairs(ch) do
						ic=ic+1; if ic%YIELD_EVERY==0 then task.wait() end
						dumpInst(c,1,"  ")
					end
				end
				w("")
		...
		if Settings.Dump.DumpLighting then
			local ok,lt=pcall(function() return game:GetService("Lighting") end)
			if ok and lt then
				hdr("LIGHTING")
				for _,pn in ipairs({"Ambient","Brightness","ClockTime","FogColor","FogEnd","FogStart","GlobalShadows","OutdoorAmbient","TimeOfDay"}) do
					local pok,pv=pcall(function() return lt[pn] end); if pok then w("  "..pn.."="..sv(pv)) end
				end
				w("")
			end
		end
		...
		if Settings.Dump.DumpTeams then
			...
			if ok and teams then
				hdr("TEAMS")
				local ok2,tl=pcall(function() return teams:GetTeams() end)
				if ok2 and tl then
					for _,team in ipairs(tl) do
						w("  "..team.Name)
						local ok3,members=pcall(function() return team:GetPlayers() end)
						if ok3 and members then for _,p in ipairs(members) do w("    "..p.Name) end end
					end
				end
				w("")
			end
		...
		if Settings.Dump.DumpTags then
			local ok,cs=pcall(function() return game:GetService("CollectionService") end)
			if ok and cs then
				hdr("COLLECTION TAGS")
				local ok2,tags=pcall(function() return cs:GetAllTags() end)
				if ok2 and tags then
					for _,tag in ipairs(tags) do
						local ok3,tagged=pcall(function() return cs:GetTagged(tag) end)
						if ok3 and tagged then
							w("  ["..tag.."] x"..#tagged)
							for _,inst in ipairs(tagged) do
								local ok4,fn=pcall(function() return inst:GetFullName() end)
								if ok4 then w("    "..fn) end
							end
						end
					end
				end
		...
		if Settings.Dump.DumpSounds and #sounds>0 then
			hdr("SOUNDS ("..#sounds..")")
				...
				for _,obj in ipairs(gc) do
					if typeof(obj)=="Instance" then
						...
						if ok2 and (cn=="LocalScript" or cn=="Script" or cn=="ModuleScript") then
							local ok3,par=pcall(function() return obj.Parent end)
							if ok3 and par==nil then
								local ok4,fn=pcall(function() return obj:GetFullName() end)
								w("  [GC "..cn.."] "..(ok4 and fn or "?")); found=found+1
							end
						end
					end
				end
		...
		if Settings.Dump.ScanNilInstances and env.getnilinstances then
			hdr("NIL-PARENTED INSTANCES")
			local ok,nilInsts=pcall(env.getnilinstances)
			if ok and nilInsts then
				local found=0
				for _,obj in ipairs(nilInsts) do
					local ok2,cn=pcall(function() return obj.ClassName end)
					local ok3,nm=pcall(function() return obj.Name end)
					if ok2 and ok3 then w("  ["..cn.."] "..nm); found=found+1 end
				end
				if found==0 then w("  (none)") end
			end
			w("")
		...
		if Settings.Dump.ScanLoadedModules and env.getloadedmodules then
			...
			if ok and mods then
				for _,mod in ipairs(mods) do
					local ok2,fn=pcall(function() return mod:GetFullName() end); if ok2 then w("  "..fn) end
				end
			end
			w("")
		end

		if Settings.Dump.SaveScripts and #scripts>0 then
			hdr("SCRIPTS ("..#scripts.." found)")
			for _,inst in ipairs(scripts) do
				local fp,sz=saveScript(inst)
				local ok2,fn=pcall(function() return inst:GetFullName() end)
				if fp then w("  SAVED: "..(ok2 and fn or inst.Name).." ("..sz.." bytes)")
				else w("  SKIP: "..(ok2 and fn or inst.Name)) end
			end
			w("")
		end
	...
	Main.Init = function()
		...
		if Main.Elevated and env.writefile and not Main.LocalDepsUpToDate() then
			pcall(env.writefile,"dex/deps_version.dat",Main.ClientVersion.."\n"..Main.RobloxVersion)
			pcall(env.writefile,"dex/rbx_api.dat",Main.RawAPI or "")
			pcall(env.writefile,"dex/rbx_rmd.dat",Main.RawRMD or "")
		end
...
xpcall(Main.Init, function(e)
	local msg = tostring(e)
	warn("[DEX CRASH] "..msg)
	pcall(function()
		local holder = game:GetService("Players").LocalPlayer
			and game:GetService("Players").LocalPlayer:FindFirstChildOfClass("PlayerGui")
			or game:GetService("CoreGui")
		local sg = Instance.new("ScreenGui"); sg.Name="DexError"; sg.ResetOnSpawn=false; sg.IgnoreGuiInset=true; sg.Parent=holder
		local fr = Instance.new("Frame",sg); fr.Size=UDim2.new(1,0,1,0); fr.BackgroundColor3=Color3.fromRGB(20,20,20); fr.BorderSizePixel=0
		local lbl = Instance.new("TextLabel",fr)
		lbl.Size=UDim2.new(1,-20,1,-20); lbl.Position=UDim2.new(0,10,0,10)
		lbl.BackgroundTransparency=1; lbl.TextColor3=Color3.fromRGB(255,80,80)
		lbl.Font=Enum.Font.Code; lbl.TextSize=14; lbl.TextWrapped=true
