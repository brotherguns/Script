...
Main = (function()
	...
	Main.DumpGame = function()
		...
		lines[#lines+1] = "=============================="
		lines[#lines+1] = "  DEX - CORE GAME FUNCTION DUMP"
		lines[#lines+1] = "  PlaceId : " .. tostring(game.PlaceId)
		lines[#lines+1] = "  Tick    : " .. timestamp
		lines[#lines+1] = "=============================="
		lines[#lines+1] = ""

		-- Classes considered core interactive/gameplay elements
		local interactiveClasses = {
			TextButton = true, ImageButton = true,
			TextLabel = true, TextBox = true,
			Frame = true, ScreenGui = true, BillboardGui = true, SurfaceGui = true,
		}

		-- Only dump key properties per class to keep it readable
		local keyProps = {
			TextButton  = {"Name","Text","Visible","BackgroundColor3","BackgroundTransparency","Position","Size"},
			ImageButton = {"Name","Image","Visible","BackgroundColor3","BackgroundTransparency","Position","Size"},
			TextLabel   = {"Name","Text","Visible","BackgroundColor3","TextColor3","BackgroundTransparency"},
			TextBox     = {"Name","PlaceholderText","Text","Visible"},
			Frame       = {"Name","Visible","BackgroundColor3","BackgroundTransparency","Position","Size"},
			ScreenGui   = {"Name","Enabled"},
			BillboardGui = {"Name","Enabled","AlwaysOnTop"},
			SurfaceGui  = {"Name","Enabled"},
		}
		...
		-- Recursively scan, only printing interactive nodes
		local function scan(inst, depth)
			local cls = inst.ClassName
			if not interactiveClasses[cls] then
				-- Still recurse in case children are interactive
				local ok, children = pcall(function() return inst:GetChildren() end)
				if ok then for _, c in ipairs(children) do scan(c, depth) end end
				return
			end

			local pad = string.rep("  ", depth)
			local nameOk, instName = pcall(function() return inst.Name end)
			lines[#lines+1] = pad .. "[" .. cls .. "] " .. (nameOk and instName or "?")

			-- Full path for easy reference
			local pathOk, fullPath = pcall(function() return inst:GetFullName() end)
			if pathOk then
				lines[#lines+1] = pad .. "  PATH: " .. fullPath
			end

			-- Dump only the relevant props for this class
			local props = keyProps[cls] or {}
			for _, prop in ipairs(props) do
				local ok, val = pcall(function() return inst[prop] end)
				if ok and val ~= nil then
					lines[#lines+1] = pad .. "  " .. prop .. " = " .. serializeVal(val)
				end
			end

			-- Flag buttons with their click connections (connection count)
			if cls == "TextButton" or cls == "ImageButton" then
				local ok, conns = pcall(function()
					return #inst.MouseButton1Click:GetConnectionCount() > 0
				end)
				if ok then
					lines[#lines+1] = pad .. "  HAS_CLICK_HANDLER = " .. tostring(ok)
				end
			end
			...
			local ok, children = pcall(function() return inst:GetChildren() end)
			if ok then for _, c in ipairs(children) do scan(c, depth + 1) end end
		end

		-- ── Section 1: PlayerGui (all interactive UI) ──
		lines[#lines+1] = "══════════════════════════════"
		lines[#lines+1] = "  PLAYER GUI (Buttons / Labels)"
		lines[#lines+1] = "══════════════════════════════"
		local pgOk, pg = pcall(function() return plr.PlayerGui end)
		if pgOk and pg then scan(pg, 0) end

		-- ── Section 2: CoreGui ──
		lines[#lines+1] = "══════════════════════════════"
		lines[#lines+1] = "  CORE GUI"
		lines[#lines+1] = "══════════════════════════════"
		local cgOk, cg = pcall(function() return service.CoreGui end)
		if cgOk and cg then scan(cg, 0) end

		-- ── Section 3: BillboardGuis / SurfaceGuis on workspace objects ──
		lines[#lines+1] = "══════════════════════════════"
		lines[#lines+1] = "  WORLD GUIS (Billboard/Surface on Parts)"
		lines[#lines+1] = "══════════════════════════════"
		local wsOk, ws = pcall(function() return workspace:GetDescendants() end)
		if wsOk then
		...
		-- ── Section 4: RemoteEvents / RemoteFunctions (network calls) ──
		lines[#lines+1] = "══════════════════════════════"
		lines[#lines+1] = "  REMOTES (ReplicatedStorage)"
		lines[#lines+1] = "══════════════════════════════"
		local rsOk, rs = pcall(function() return service.ReplicatedStorage:GetDescendants() end)
		if rsOk then
			for _, obj in ipairs(rs) do
				if obj:IsA("RemoteEvent") or obj:IsA("RemoteFunction") or obj:IsA("BindableEvent") then
					local pathOk, path = pcall(function() return obj:GetFullName() end)
					lines[#lines+1] = "[" .. obj.ClassName .. "] " .. (pathOk and path or obj.Name)
				end
			end
		end

		-- ── Section 5: LocalScripts ──
		lines[#lines+1] = ""
		lines[#lines+1] = "══════════════════════════════"
		lines[#lines+1] = "  LOCAL SCRIPTS"
		lines[#lines+1] = "══════════════════════════════"
		local function findScripts(parent)
			local ok, descs = pcall(function() return parent:GetDescendants() end)
			...
			for _, obj in ipairs(descs) do
				if obj:IsA("LocalScript") or obj:IsA("ModuleScript") then
					...
					if env.decompile then
						local ds, dv = pcall(env.decompile, obj)
						if ds and dv then src = " [decompiled: " .. tostring(#dv) .. " chars]" end
					end
					lines[#lines+1] = "[" .. obj.ClassName .. "] " .. (pathOk and path or obj.Name) .. src
				end
			end
		end
		findScripts(plr.PlayerGui)
		pcall(findScripts, plr.Backpack)
		pcall(findScripts, service.ReplicatedStorage)

		local content = table.concat(lines, "\n")
		env.writefile(txtFile, content)
		print("[Dex Dump] Core game dump saved to: " .. txtFile)
	end
