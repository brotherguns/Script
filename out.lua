...
--[[
	...
	Iteratively traverses game, CoreGui (implicitly via game), and
	nil-parented instances (via getnilinstances when available).
	Decompiles every LocalScript / ModuleScript / Script via the
	executor's decompile() API, and mirrors the Explorer tree on disk
	using writefile() / makefolder().

	Design notes:
		- Iterative BFS with an explicit queue; no deep recursion, so
		  stack depth is bounded regardless of tree shape. This was
		  specifically changed from the previous recursive version that
		  crashed mobile executors on games with ~90k+ instances.
		- Yields to the scheduler every N instances so the game thread
		  doesn't stall and the executor doesn't kill the script.
		- Log display is capped and writes directly; no 500-line string
		  rebuilds per message.
		- Info sidecar files are OFF by default — they 10x the file
		  count on large games and most users only want scripts.
		- Orphan sweep via getinstances() is OFF by default for the
		  same reason (it often duplicates what we already dumped).
]]
...
local function main()
	...
	local INFO_PROPERTIES = {
		...
		"Anchored", "CanCollide", "Material", "Transparency",
		"Visible", "Text", "ZIndex",
		"Disabled", "RunContext",
		"MeshId", "TextureId", "AssetId", "SoundId",
		"Value",
	}
	...
	-- Safe property reads
	--
	-- Factored out as top-level functions so pcall captures a stable
	-- upvalue rather than allocating a fresh closure per call. On a
	-- game with 90k instances and 6+ property reads each, that's
	-- 500k+ closure allocations we avoid per dump run.
	-------------------------------------------------------------------

	local function _readIndex(inst, prop) return inst[prop] end
	local function _getChildren(inst) return inst:GetChildren() end
	...
	-------------------------------------------------------------------
	-- Info-file serialization
	-------------------------------------------------------------------

	local function valueToString(v)
		local t = typeof(v)
		if t == "Instance" then
			local ok, full = pcall(_getFullName, v)
	...
	local function buildInfoText(inst, fullPath)
		local parts = { "-- Dex Dump Info File", "FullName: " .. fullPath }
		for i = 1, #INFO_PROPERTIES do
			local prop = INFO_PROPERTIES[i]
			local val = safeGet(inst, prop)
			if val ~= nil then
				parts[#parts + 1] = prop .. " = " .. valueToString(val)
			end
		end
	...
	local function extractScriptSource(inst, decompile)
		...
		if decompile then
			...
			if not okDec then
				return "-- Dex Dumper: decompile() errored: " .. tostring(decSrc), "error"
			end
		end
	...
	-------------------------------------------------------------------
	-- Core: iterative BFS traversal
	--
	-- Uses explicit head/tail indices instead of table.remove(q, 1)
	-- so dequeue is O(1). With 100k+ instances, table.remove-at-1
		...
		if nilList then
			...
			for i = 1, #nilList do
				...
				if inst and inst ~= game then
					counter = counter + 1
					local cn = safeGet(inst, "ClassName") or "Unknown"
					local nm = safeGet(inst, "Name") or ""
					local folderName = string.format("%04d_%s_%s",
						counter, sanitizeComponent(cn), sanitizeComponent(nm))
					roots[#roots + 1] = {
						inst = inst,
						path = nilRoot .. "/" .. folderName,
					}
				end
			end
		else
		...
		if not cancelRequested and doOrphanSweep then
			...
			if all then
				...
				for i = 1, #all do
					...
					if inst then
						local cn = safeGet(inst, "ClassName") or "Unknown"
						orphanRoots[#orphanRoots + 1] = {
							inst = inst,
							path = string.format("%s/%05d_%s",
								sweepRoot, i, sanitizeComponent(cn)),
						}
					end
				end
	...
	Dumper.Init = function()
		...
		desc.Text = "Dumps game + nil instances to dex/dumps/<PlaceId>_<time>/. "
		...
		pathBtn.MouseButton1Click:Connect(function()
			local path = string.format("dex/dumps/%s_*",
