...
Main = (function()
	...
	Main.DumpGame = function()
		...
		local rbxlFile = "dex/saved/dump_" .. timestamp .. ".rbxl"

		-- Header
		lines[#lines+1] = "=============================="
		lines[#lines+1] = "  DEX GAME DUMP"
		...
		-- Serialize a single value to a readable string
		local function serializeVal(val)
			...
			elseif t == "BrickColor" then return 'BrickColor("' .. tostring(val) .. '")'
			...
			elseif t == "UDim"     then return ("UDim(%g,%g)"):format(val.Scale,val.Offset)
			elseif t == "Rect"     then return ("Rect(%g,%g,%g,%g)"):format(val.Min.X,val.Min.Y,val.Max.X,val.Max.Y)
			elseif t == "NumberRange" then return ("NumberRange(%g,%g)"):format(val.Min,val.Max)
			elseif t == "EnumItem" then return tostring(val)
			elseif t == "Instance" then return "[Instance] " .. (pcall(function() return val:GetFullName() end) and val:GetFullName() or val.Name)
			else return "[" .. t .. "]" end
		end

		-- Recursively dump an instance tree
		local function dumpInstance(inst, depth)
			local pad = string.rep("  ", depth)
			lines[#lines+1] = pad .. "[" .. inst.ClassName .. "] " .. inst.Name

			-- Dump properties via the loaded API if available
			if API and API.GetMember then
				local props = API.GetMember(inst.ClassName, "Properties")
				if props then
					local propPad = pad .. "  | "
					for _, prop in ipairs(props) do
						if not prop.Tags.WriteOnly then
							local ok, val = pcall(function() return inst[prop.Name] end)
							if ok and val ~= nil then
								lines[#lines+1] = propPad .. prop.Name .. " = " .. serializeVal(val)
							end
						end
					end
				end
		...
		-- Walk every top-level service
		dumpInstance(game, 0)

		local content = table.concat(lines, "\n")
		env.writefile(txtFile, content)

		-- Also attempt a binary saveinstance dump alongside the text file
		if env.saveinstance then
			pcall(env.saveinstance, game, rbxlFile)
		end

		print("[Dex Dump] Text dump saved to: " .. txtFile)
		if env.saveinstance then
			print("[Dex Dump] Binary dump saved to: " .. rbxlFile)
		end
	end
